...
const MarkdownRenderer = ({ content }) => {
  ...
  const sanitizationSchema = {
    ...
    tagNames: [
      ...
      'ul', 'ol', 'li', 'a', 'br', 'img', 'hr',
      'table', 'thead', 'tbody', 'tr', 'th', 'td',
      'h1', 'h2', 'h3', 'h4', 'h5', 'h6',
      'math', 'semantics', 'mrow', 'mi', 'mo', 'mn', 'msup', 'mfrac',
      'munder', 'mover', 'mtable', 'mtr', 'mtd', 'msqrt', 'mroot',
      'mtext', 'menclose', 'mglyph', 'maligngroup', 'malignmark',
      'mpadded', 'mphantom', 'mspace', 'mstyle', 'msub', 'msubsup',
      'ms', 'mstack', 'mlongdiv', 'msline', 'maction', 'annotation',
      'annotation-xml', 'span'
    ],
    protocols: {
  ...
  return (
    <div className="markdown-body">
      <ReactMarkdown
        ...
        rehypePlugins={[[rehypeKatex, { output: 'mathml' }], [rehypeSanitize, sanitizationSchema]]}
        components={{
          ...
          // Styled tables
          table({ children }) {
            return <table className="my-4 border-collapse border border-gray-300">{children}</table>;
          },
          th({ children, align }) {
            return (
              <th 
          ...
          h1({ children }) {
            return <h1 className="text-3xl font-bold my-6 pb-2 border-b">{children}</h1>;
          },
          h2({ children }) {
            return <h2 className="text-2xl font-semibold my-5 pb-2 border-b">{children}</h2>;
          },
