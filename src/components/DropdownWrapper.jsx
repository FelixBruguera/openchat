...
const DropdownWrapper = ({ selectedModel }) => {
    ...
    <DropdownMenu>
        ...
        <DropdownMenuContent>
            <DropdownMenuRadioGroup value={selectedModel.id} onValueChange={(e) => dispatch(setSelectedModel(e.target))}></DropdownMenuRadioGroup>
            <DropdownMenuLabel>Favorite models</DropdownMenuLabel>
            <DropdownMenuSeparator />
            { favorites.length > 0 
                ? favorites.map(favorite => 
                <DropdownMenuRadioItem value={favorite.id}>{favorite.name}</DropdownMenuRadioItem>
                )
